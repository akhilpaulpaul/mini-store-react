...
const TopSellers = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const authoriseData: Author[] = data.results.map((user: any) => ({
          name: `${user.name.first} ${user.name.last}`,
          isFollowing: false,
          image: user.picture.medium,
        }));
        setAuthors(authoriseData);
      } catch (error) {
  ...
  return (
    <div className="bg-white p-5 mx-5 mt-4 border w-[23rem] rounded">
